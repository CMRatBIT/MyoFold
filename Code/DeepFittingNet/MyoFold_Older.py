...
if torch.cuda.is_available():
    # myoRelaxationNet = torch.nn.DataParallel(myoRelaxationNet, device_ids=params.device_ids[0]).cuda()
...
def TestingInvivo(net, loadDataPath,TxDispMax, worksheet , xlsxRowrow,xlsxCol , seqTag = 'MOLLI53',LLCorrection=False):
    try:
        ...
        for ix in range(0,sigsTisTsatsTEs_.shape[0]):
            ...
            # predA = predA_[ix, :, :, :].squeeze()
            # predB = predB_[ix, :, :, :].squeeze()
        ...
        print(e)
    return stRow
workbook = xlsxwriter.Workbook( os.path.normpath(cwdpath.parent.parent)+'//Results//Older//TestingInvivo_MyoFold.xlsx')
worksheet = workbook.add_worksheet()

xlsxRow = 0
xlsxCol = 0

xlsxRow = TestingInvivo(myoRelaxationNet,os.path.normpath(cwdpath.parent.parent)+'//data//Older//MyoFold.mat',2000,worksheet, xlsxRow,xlsxCol,'MyoFold',LLCorrection=False)
